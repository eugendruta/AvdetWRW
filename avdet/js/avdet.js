$(document).ready(function () {
  ...
  // $('#mandantlb').append($('<option></option>').val(0).html('initial'))  
      ...
      for (let i = 0; i < config.default.data.listboxen.length; i++) {
        if (config.default.data.listboxen[i].name === lbname) {
          ...
          $.getJSON(url, function (data) {
            ...
            if (lbname == 'mandantlb') {
              var folgeparam = localStorage.getItem(dialogname + '.param')
              if (folgeparam) {
                UTIL.logger(dialogname + ': localStorage.avdet.param.mandant: '
                  + jQuery.parseJSON(folgeparam).mandant
                  + '; auftnr: ' + jQuery.parseJSON(folgeparam).lenr)

                // Setze Inputfelder
                $('#auftrag').val(jQuery.parseJSON(folgeparam).lenr)              
                $("#mandantlb").val(jQuery.parseJSON(folgeparam).mandant)
                console.log('initlb(): selected val: ' + $('#mandantlb').val())                
              }
            }
          })
        }
  ...
  changelb = function changelb (lbname, value) {
    ...
    $('#' + lbname).val(value)
    UTIL.logger(dialogname + ': changelb(): val(): ' + $('#' + lbname).val())
    return
  ...
  // Hole Parameter vom Vater
  /*
  var folgeparam = localStorage.getItem(dialogname + '.param')
  if (folgeparam) {
    UTIL.logger(dialogname + ': avdet(): localStorage.avdet.param.mandant: '
      + jQuery.parseJSON(folgeparam).mandant
      + '; auftnr: ' + jQuery.parseJSON(folgeparam).lenr)
    ...
    //$("#mandantlb").val(jQuery.parseJSON(folgeparam).mandant)
    //$("#mandantlb").val('MAN-int')
    // document.getElementById('mandantlb').value = 'MAN-efa'          
    $("#mandantlb option[value='MAN-int']").attr("selected", "selected");           
    UTIL.logger(dialogname + ': avdet(): val(): ' + $("#mandantlb").val())

    /*$('#mandantlb').empty().append($('<option></option>').val('0').html('Alle'))
    $("#mandantlb option[value='MAN-int']").attr("disabled", false);    
    $('#mandantlb').val('MAN-int')        
    $('#mandantlb option').eq(2).val('MAN-int')
    UTIL.logger(dialogname + ': ready() #11: val(1): ' + $('#mandantlb option').eq(0).val())
    $('#mandantlb').val($('#mandantlb option').eq(2).val())
    UTIL.logger(dialogname + ': ready() #2: val(): ' + $('#mandantlb').val())
    $('#auftrag').val(jQuery.parseJSON(folgeparam).lenr)
    $('#mandantlb').val('MAN-efa') 
    //

  // $('#auftrag').val(jQuery.parseJSON(folgeparam).lenr)
  // document.getElementById('mandantlb').value = 'MAN-efa'
  
  //$('mandantlb select').val('MAN-efa')
  //UTIL.logger(dialogname + ': ready(): ende: value: ' +document.getElementById('mandantlb').html)
  }
  */
